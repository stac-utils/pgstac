...
SELECT results_eq($$
    SELECT properties->>'eo:cloud_cover' FROM items WHERE collection='pgstac-test-collection';
    $$,$$
    SELECT '28';
    $$,
...
-- merge_jsonb and strip_jsonb must preserve explicit JSON null values.
SELECT results_eq(
    $$ SELECT merge_jsonb(
        '{"properties": {"datetime": null, "start_datetime": "2026-01-01T00:00:00Z", "end_datetime": "2026-01-31T23:00:00Z"}}'::jsonb,
        '{"properties": {"datetime": "2025-12-31T23:59:59Z"}}'::jsonb
    )->'properties'->'datetime' $$,
    $$ SELECT 'null'::jsonb $$,
    'merge_jsonb preserves explicit JSON null values over inherited base values'
);
...
SELECT results_eq(
    ...
    $$ SELECT 'null'::jsonb $$,
    'strip_jsonb preserves explicit JSON null values over inherited base values'
);
