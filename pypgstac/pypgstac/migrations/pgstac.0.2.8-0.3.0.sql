...
drop function if exists "pgstac"."filter_by_order"(_item pgstac.item, _sort jsonb, _type text);

drop function if exists "pgstac"."filter_by_order"(item_id text, _sort jsonb, _type text);

drop function if exists "pgstac"."flip_jsonb_array"(j jsonb);
...
CREATE INDEX properties_idx ON ONLY pgstac.items USING gin (pgstac.properties_idx((content -> 'properties'::text)));
...
CREATE OR REPLACE FUNCTION pgstac.properties_idx(jdata jsonb)
 RETURNS jsonb
 LANGUAGE sql
 IMMUTABLE PARALLEL SAFE
AS $function$
